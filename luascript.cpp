...
void LuaInterface::registerFunctions()
{
	...
	lua_register(m_luaState, "doRemoveInvisible", LuaInterface::luaDoRemoveInvisible);

	lua_register(m_luaState, "doSetInvisible", LuaInterface::luaDoSetInvisible);
...
int32_t LuaInterface::luaDoSetInvisible(lua_State* L)
{
    std::cout << "[luaDoSetInvisible] Iniciando funcao." << std::endl;

    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if (!player) {
        std::cout << "[luaDoSetInvisible] Player nao encontrado." << std::endl;
        lua_pushboolean(L, false);
        return 1;
    }

    Condition* condition = Condition::createCondition(CONDITIONID_DEFAULT, CONDITION_CUSTOM, -1);
    if (!condition) {
        std::cout << "[luaDoSetInvisible] Falha ao criar a condition." << std::endl;
        lua_pushboolean(L, false);
        return 1;
    }

    player->addCondition(condition);
    std::cout << "[luaDoSetInvisible] Condition adicionada." << std::endl;
    ...
    std::cout << "[luaDoSetInvisible] Status online atualizado para false." << std::endl;

    std::cout << "[luaDoSetInvisible] Forcando redesenho com teleport." << std::endl;
    g_game.internalTeleport(player, player->getPosition(), false);

    std::cout << "[luaDoSetInvisible] Invisibilidade aplicada com sucesso!" << std::endl;
    lua_pushboolean(L, true);
    return 1;
}

int32_t LuaInterface::luaDoRemoveInvisible(lua_State* L)
{
    std::cout << "[luaDoRemoveInvisible] Iniciando funcao." << std::endl;

    ScriptEnviroment* env = getEnv();
    Player* player = env->getPlayerByUID(popNumber(L));
    if (!player) {
        std::cout << "[luaDoRemoveInvisible] Player nao encontrado." << std::endl;
        lua_pushboolean(L, false);
        return 1;
    }

    player->removeCondition(CONDITION_CUSTOM);

    if (!player->hasCondition(CONDITION_CUSTOM)) {
        std::cout << "[luaDoRemoveInvisible] Condition removida com sucesso." << std::endl;
    } else {
        std::cout << "[luaDoRemoveInvisible] Falha ao remover a condition." << std::endl;
    }

    // Atualiza estado interno de visibilidade
    g_game.internalCreatureChangeVisible(player, VISIBLE_GHOST_APPEAR);

    // Atualiza status de online no banco
    IOLoginData::getInstance()->updateOnlineStatus(player->getGUID(), true);

    // Notifica espectadores que o jogador apareceu
    SpectatorVec list;
    g_game.getSpectators(list, player->getPosition(), true, true);

    for (Creature* spectator : list) {
        Player* specPlayer = spectator->getPlayer();
        if (specPlayer && specPlayer != player) {
            specPlayer->sendCreatureAppear(player);
        }
    }

    // Notifica o próprio jogador sobre o novo visual (opcional, pode já estar sincronizado)
    player->sendCreatureAppear(player);

    std::cout << "[luaDoRemoveInvisible] Invisibilidade removida com sucesso!" << std::endl;
    lua_pushboolean(L, true);
    return 1;
}
