...
bool Player::canSeeCreature(const Creature* creature) const
{
	...
	if (const Player* player = creature->getPlayer()) {
		// Se tiver CONDITION_CUSTOM, é invisível.
		if (player->hasCondition(CONDITION_CUSTOM))
			return false;

		// Lógica original de ghost
		return !player->isGhost() || getGhostAccess() >= player->getGhostAccess();
	}

	// Lógica padrão para monstros ou NPCs
...
void Player::sendCreatureChangeVisible(const Creature* creature, Visible_t visible)
{
	...
	const Player* player = creature->getPlayer();
	const bool hasCustomCondition = player && player->hasCondition(CONDITION_CUSTOM);

	if (player == this || 
	    (player && !hasCustomCondition && (visible < VISIBLE_GHOST_APPEAR || getGhostAccess() >= player->getGhostAccess())) ||
	    (!player && canSeeInvisibility()))
	{
		sendCreatureChangeOutfit(creature, creature->getCurrentOutfit());
	}
	else if (visible == VISIBLE_DISAPPEAR || visible == VISIBLE_GHOST_DISAPPEAR || hasCustomCondition)
	{
...
void Player::removeList()
{
	...
	autoList.erase(id);
	if(!isGhost() && !hasCondition(CONDITION_CUSTOM))
	{
		for(AutoList<Player>::iterator it = autoList.begin(); it != autoList.end(); ++it)
...
uint32_t Player::getAttackSpeed() const
{
	...
	// 1. Verifica modificadores de outfit (se houver)
	if (outfitAttributes) {
		Outfit outfit;
		if (Outfits::getInstance()->getOutfit(defaultOutfit.lookType, outfit)) {
			if (outfit.attackSpeed == -1)
				return 0; // outfit bloqueia ataques
	...
	modifiers += static_cast<int32_t>(extraAttackSpeed);

	if (modifiers == 9999) {
	return 0;
	}
	
	uint32_t baseAttackSpeed = 0;

	
		// Nenhuma arma válida: usa a lógica baseada em skill de Fist
		int32_t fistSkill = getSkill(SKILL_FIST, SKILL_LEVEL);

		if (fistSkill < 20)
			baseAttackSpeed = 1700;
		else if (fistSkill < 30)
			baseAttackSpeed = 1600;
		else if (fistSkill < 40)
			baseAttackSpeed = 1500;
		else if (fistSkill < 50)
			baseAttackSpeed = 1400;
		else if (fistSkill < 60)
			baseAttackSpeed = 1300;
		else if (fistSkill < 70)
			baseAttackSpeed = 1200;
		else if (fistSkill < 80)
			baseAttackSpeed = 1100;
		else if (fistSkill < 99)
			baseAttackSpeed = 1000;
		else
			baseAttackSpeed = 900;
	
	if (static_cast<int32_t>(baseAttackSpeed) - modifiers <= 0)
		return 100; // valor mínimo de attack speed

	return baseAttackSpeed - modifiers;
}
