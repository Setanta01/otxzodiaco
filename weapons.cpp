...
bool Weapon::useFist(Player* player, Creature* target)
{
	...
	int32_t fistSkill = player->getSkill(SKILL_FIST, SKILL_LEVEL);
	int CRITICALCHANBEBYFIST;
	if (fistSkill < 100)
		CRITICALCHANBEBYFIST = 5;
	else if (fistSkill < 110)
		CRITICALCHANBEBYFIST = 10;
	else if (fistSkill < 120)
		CRITICALCHANBEBYFIST = 15;
	else if (fistSkill < 130)
		CRITICALCHANBEBYFIST = 20;
	else if (fistSkill < 140)
		CRITICALCHANBEBYFIST = 25;
	else if (fistSkill < 150)
		CRITICALCHANBEBYFIST = 30;
	else if (fistSkill < 160)
		CRITICALCHANBEBYFIST = 35;
	else if (fistSkill < 170)
		CRITICALCHANBEBYFIST = 40;
	else if (fistSkill < 180)
		CRITICALCHANBEBYFIST = 45;
	else if (fistSkill < 190)
		CRITICALCHANBEBYFIST = 50;
	else if (fistSkill < 200)
		CRITICALCHANBEBYFIST = 55;
	else
		CRITICALCHANBEBYFIST = 60;

	if (player->getCriticalHitChance() + CRITICALCHANBEBYFIST >= random_range(1, 100)) {
...
int32_t WeaponMelee::getWeaponDamage(const Player* player, const Creature*, const Item* item, bool& isCritical, bool maxDamage /*= false*/) const
{
	...
	int32_t fistSkill = player->getSkill(SKILL_FIST, SKILL_LEVEL);
	// Definindo chance crítica baseada no fistSkill
