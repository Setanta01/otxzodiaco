...
bool ProtocolGame::canSee(uint16_t x, uint16_t y, uint16_t z) const
{
	...
	bool isOTClient = player->isUsingOtclient();

	return ((x >= myPos.x - Map::getViewportX(isOTClient) + offsetz) && (x <= myPos.x + (Map::getViewportX(isOTClient)+1) + offsetz) &&
     (y >= myPos.y - Map::getViewportY(isOTClient) + offsetz) && (y <= myPos.y + (Map::getViewportY(isOTClient)+1) + offsetz));
}
	...
	bool isOTClient = player->isUsingOtclient();
		...
		else
		{
			...
			if(oldPos.y > newPos.y) // north, for old x
			{
				...
				GetMapDescription(oldPos.x - Map::getViewportX(isOTClient), newPos.y - Map::getViewportY(isOTClient), newPos.z, (Map::getViewportX(isOTClient)+1)*2, 1, msg);
			}
			else if(oldPos.y < newPos.y) // south, for old x
			{
				...
				TRACK_MESSAGE(msg);
				msg->addByte(0x67);
				GetMapDescription(oldPos.x - Map::getViewportX(isOTClient), newPos.y + (Map::getViewportY(isOTClient)+1), newPos.z, (Map::getViewportX(isOTClient)+1)*2, 1, msg);
			}
			...
			if(oldPos.x < newPos.x) // east, [with new y]
			{
				...
				TRACK_MESSAGE(msg);
				msg->addByte(0x66);
				GetMapDescription(newPos.x + (Map::getViewportX(isOTClient)+1), newPos.y - Map::getViewportY(isOTClient), newPos.z, 1, (Map::getViewportY(isOTClient)+1)*2, msg);
			}
			else if(oldPos.x > newPos.x) // west, [with new y]
			{
				...
				TRACK_MESSAGE(msg);
				msg->addByte(0x68);
				GetMapDescription(newPos.x - Map::getViewportX(isOTClient), newPos.y - Map::getViewportY(isOTClient), newPos.z, 1, (Map::getViewportY(isOTClient)+1)*2, msg);
			}
		}
	}
...
void ProtocolGame::AddMapDescription(OutputMessage_ptr msg, const Position& pos)
{
	...
	GetMapDescription(pos.x - Map::getViewportX(isOTClient), pos.y - Map::getViewportY(isOTClient), pos.z, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, msg);
}
	...
	const Position& newPos, const Position& oldPos, uint32_t)
{
	...
	if(newPos.z == 7) //going to surface
	{
		...
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 5, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 3, skip); //(floor 7 and 6 already set)
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 4, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 4, skip);
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 3, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 5, skip);
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 2, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 6, skip);
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 1, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 7, skip);
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), 0, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 8, skip);
		if(skip >= 0)
		{
			msg->addByte(skip);
			msg->addByte(0xFF);
	...
	else if(newPos.z > 7) //underground, going one floor up (still underground)
	{
		...
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), oldPos.z - 3, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, 3, skip);
		if(skip >= 0)
		{
			msg->addByte(skip);
			msg->addByte(0xFF);
	...
	GetMapDescription(oldPos.x - Map::getViewportX(isOTClient), oldPos.y - (Map::getViewportY(isOTClient)-1), newPos.z, 1, (Map::getViewportY(isOTClient)+1)*2, msg);

	//north
	msg->addByte(0x65);
	
	GetMapDescription(oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), newPos.z, (Map::getViewportX(isOTClient)+1)*2, 1, msg);
}
	...
	const Position& newPos, const Position& oldPos, uint32_t)
{
	...
	if(newPos.z == 8) //going from surface to underground
	{
		...
		GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), newPos.z, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, -1, skip);
        GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), newPos.z + 1, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, -2, skip);
        GetFloorDescription(msg, oldPos.x - Map::getViewportX(isOTClient), oldPos.y - Map::getViewportY(isOTClient), newPos.z + 2, (Map::getViewportX(isOTClient)+1)*2, (Map::getViewportY(isOTClient)+1)*2, -3, skip);
		if(skip >= 0)
		{
			msg->addByte(skip);
			msg->addByte(0xFF);
	...
	GetMapDescription(oldPos.x + Map::getViewportX(isOTClient)+1, oldPos.y - (Map::getViewportY(isOTClient)+1), newPos.z, 1, ((Map::getViewportY(isOTClient)+1)*2), msg);

	//south
	msg->addByte(0x67);
	
	GetMapDescription(oldPos.x - Map::getViewportX(isOTClient), oldPos.y + (Map::getViewportY(isOTClient)+1), newPos.z, ((Map::getViewportX(isOTClient)+1)*2), 1, msg);
}
